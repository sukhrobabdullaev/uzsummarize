...
export function ImageDescription() {
    ...
    const processFile = (file: File) => {
        ...
        setResult("")
        setError(null)
        toast.success(`${file.name} is ready to process`)
    }
                ...
                >
                    {!previewUrl ? (
                        <>
                            ...
                            <h3 className="text-lg md:text-xl font-medium mb-1">{t('imageDescription.upload.dragDrop')}</h3>
                            <p className="text-sm md:text-base text-muted-foreground mb-4">
                                {t('imageDescription.upload.browseFiles')}
                            </p>
                            <Button variant="outline" className="relative">
                                {t('imageDescription.upload.chooseFile')}
                                <input
                                    type="file"
                                    accept="image/*"
                ...
                >
                    {loading ? (
                        <>
                            ...
                            {t('imageDescription.processing')}
                        </>
                    ) : (
                        t('imageDescription.generateDescription')
                    )}
                </Button>

                {error && (
                    <div className="p-4 bg-destructive/10 text-destructive rounded-lg animate-in fade-in-50 duration-300">
                        <h3 className="font-semibold mb-2">{t('imageDescription.error')}</h3>
                ...
                {result && (
                    <div className="p-4 md:p-6 bg-card border rounded-lg shadow-sm animate-in fade-in-50 duration-300">
                        <div className="flex justify-between items-center mb-3">
                            <h3 className="font-semibold text-lg md:text-xl">{t('imageDescription.result.title')}</h3>
                            <Button
                                variant="ghost"
                                size="sm"
